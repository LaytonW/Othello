...
namespace Othello {
  ...
  private:
    OthelloBitBoard blackBitBoard;
    OthelloBitBoard whiteBitBoard;
    OthelloPlayer   currentPlayer;

  public:
    OthelloGame();
    const OthelloAction getActions(const OthelloState&) const override;
    const OthelloState getResult(const OthelloAction) const override;
    void performAction(OthelloState&, const OthelloAction) override;
